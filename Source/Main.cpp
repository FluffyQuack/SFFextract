...
/*
char *unknownDirName = "__UNKNOWN\\";
char *manifestFilePath = "__MANIFEST/MANIFEST.TXT";
...
int createMajorVersion = 4; //Version number to write when creating a PAK file, default matches the version of RE4 and earlier games
int createMinorVersion = 0;
int createAlwaysCompress = 0;
int createNeverCompress = 0;
bool useManifest = 1;
*/

void HelpText()
{
	printf("SFFextract v0.1 (by youtube.com/user/FluffyQuack)\n\n");
	printf("usage: SFFextract [options]\n");
	printf("  -x [SFF]		Unpack SFF container file\n");
	/*
	...
	printf("  -murmur [string]	Convert string to a 32-bit Murmur3 hash\n");
	...
	printf("  -alwaysCompress	Always compress files during PAK creation\n");
	printf("  -neverCompress	Never compress files during PAK creation\n");
	printf("  -version [4 0]	Version number to add during PAK creation\n");
	printf("  -noManifest		Skip manifest during PAK creation/extraction\n");
	*/
}
...
int _tmain(int argc, _TCHAR *argv[])
{
	...
	while(argc > i)
	{
		if(argv[i][0] == '-')
		{
			...
			else if(_stricmp(argv[i], "-alwaysCompress") == 0)
				createAlwaysCompress = 1;
			else if(_stricmp(argv[i], "-neverCompress") == 0)
				createNeverCompress = 1;
			else if(_stricmp(argv[i], "-version") == 0)
			{
				if(argc > i && IsDigit((argv[i + 1])[0]))
				{
					createMajorVersion = atoi(argv[i + 1]);
					i++;
					if(argc > i && IsDigit((argv[i + 1])[0]))
					{
						createMinorVersion = atoi(argv[i + 1]);
						i++;
					}
				}
			}
			else if(_stricmp(argv[i], "-noManifest") == 0)
				useManifest = 0;
				*/
		}
		else
	...
	{
		ReadHashList_FromFile(filenameHashList, outputNewHashList);
	...
	*/
	...
	{
		if(ReadHashList_FromFile(str2, 1))
		{
			...
			if(ReadHashList_FromFile(str1, 1))
				CombineHashLists(str2, altHashList, altHashListSize, 1);
			delete[]altHashList;
			altHashList = 0;
		}
	}
